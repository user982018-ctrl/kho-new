...
(function($){
	$.fn.selectstyle = function(option){
		...
		this.each(function(){
			...
			html = 
			'<div class="selectstyle ss_dib inline-block'+setting.theme+'" style="width:'+parseInt(setting.width + 20)+'px;">'+
				'<div id="select_style" class="ss_button" style=";width:'+parseInt(setting.width + 20)+'px;'+style+'">'+
					'<div class="inline-block ss_dib ss_text" id="select_style_text" style="margin-right:5px;width:'+(parseInt(setting.width) - 40)+'px;position:relative;">'+placeholder+'</div>'+
					'<div class="inline-block ss_dib ss_image"></div>'+
				'</div>';
			if(search == "true"){
				html += '<ul id="select_style_ul" sid="'+id+'" class="ss_ulsearch" style="max-height:'+setting.height+'px;width:'+(parseInt(setting.width) + 20)+'px;"><div class="search" id="ss_search"><input type="text" placeholder="Tìm sản phẩm..."></div><ul style="max-height:'+(parseInt(setting.height) - 53)+'px;" class="ss_ul">'+html_op+'</ul></ul>';
			}
			else{
		...
		$(document).delegate("body", "click", function(e) {
			...
			if ($(this) === $('ul#select_style_ul, div#select_style'))
